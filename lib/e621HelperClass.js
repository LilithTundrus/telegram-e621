...
class e621Helper {
    ...
    popularURLHanlder(typeArg) {
        switch (typeArg) {
            case 'daily':
                let dailyUrl = `${BASE_URL}post/popular_by_day.json`;
                return requestUrl(dailyUrl)
                    .then((response) => {
                        return response;
                    })
                    .catch((err) => {
            ...
            case 'weekly':
            case 'daily':
                let weeklyUrl = `${BASE_URL}post/popular_by_week.json`;
                return requestUrl(weeklyUrl)
                    .then((response) => {
                        return response;
                    })
                    .catch((err) => {
            ...
            case 'monthly':
            case 'daily':
                let monthlyUrl = `${BASE_URL}post/popular_by_month.json`;
                return requestUrl(monthlyUrl)
                    .then((response) => {
                        return response;
                    })
                    .catch((err) => {

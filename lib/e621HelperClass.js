...
class e621Helper {
    constructor(options) {
        // Actually assign options using object.assign
    }
    ...
    test() {
        return paginateE621EndpointAlt(`https://e621.net/post/index.json?`, 1, 3)
    }
...
function paginateE621Endpoint(urlWithoutPageNum) {
    ...
    let pageInfoArray = [];
    let endOfListBool = false;
    let i = 1;
    // create a loop and keep requesting pages until we get an empty or out_of_bounds response
    while (!endOfListBool) {
        let urlWithPageNum = `${urlWithoutPageNum}&page=${i}`;
        requestUrl(urlWithPageNum)
            .then((response) => {
...
function paginateE621EndpointAlt(urlWithoutPageNum, start, limit) {
    let resultsArray = [];
    //array contents == array contents + the new stuff AAAAAAAAAAAAAAA
    return requestUrl(`${urlWithoutPageNum}&page=${start}`)
        .then((response) => {
            logger.debug(response.length);
            if (response.length !== 0 && limit !== start) {
                return paginateE621EndpointAlt(urlWithoutPageNum, start + 1, limit)
            } else {

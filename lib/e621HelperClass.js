...
class e621Helper {
    ...
    test() {
        var dataArray = [];
        return paginateE621EndpointAlt(`https://e621.net/post/index.json?`, 1, 3, dataArray)
    }
...
function paginateE621EndpointAlt(urlWithoutPageNum, start, limit, pageArray) {
    return requestUrl(`${urlWithoutPageNum}&page=${start}`)
        .then((response) => {
            ...
            if (response.length !== 0 && limit !== start) {
                ...
                return logger.debug(`Done, got ${pageArray.length + 1} pages`)
            }
        })
}
